...
source "vsphere-clone" "kubernetes-node" {
...
build {
  sources = ["source.vsphere-clone.kubernetes-node"]
  provisioner "ansible" {
    user          = local.ssh_username
    playbook_file = "${path.cwd}/../../ansible/playbooks/kubernetes/kubernetes-node.yml"
